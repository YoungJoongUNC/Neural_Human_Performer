...
class FrameSampler(Sampler):
    ...
    def __init__(self, dataset):
        ...
        human_info = get_human_info.get_human_info(dataset.split)
        ...
        for human_idx in range(len(human_list)):
            ...
            if cfg.test_sample_cam:
                if human in ['CoreView_313', 'CoreView_315']:
                    num_cams = len(cfg.zju_313_315_sample_cam)
                else:
                    num_cams = len(cfg.zju_sample_cam)
            else:
                num_cams = len(dataset.cams[human]['K']) - len(
                    cfg.test_input_view)
            ...
            self.inds.extend(sub_inds.ravel())
            start = start + sub_len
